...
function getPlaceholderModule(Quill, options) {
    ...
    var PlaceholderModule = function PlaceholderModule(quill, options) {
        ...
        this.quill.insertPlaceholder = function (placeholder) {
            var selection = _this.quill.getSelection();
            var index = selection ? selection.index : 0;
            var length = selection ? selection.length : 0;
            _this.quill.deleteText(index, length);
            _this.quill.insertEmbed(index, 'placeholder', placeholder, Quill.sources.USER);
            _this.quill.setSelection(index + 1, 0);
        };
    };
